export default async function handler(req, res) {
  ...
  const scrapers = [
    ()=>fetch(url,{headers:{'User-Agent':'Mozilla/5.0 (compatible; Googlebot/2.1)','Accept':'text/html','Accept-Language':'fr-BE,fr;q=0.9'},signal:AbortSignal.timeout(8000)}),
    ()=>fetch(`https://corsproxy.io/?${encodeURIComponent(url)}`,{signal:AbortSignal.timeout(8000)}),
    ()=>fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`,{signal:AbortSignal.timeout(8000)}).then(async r=>{const j=await r.json();return{text:async()=>j?.contents||''}}),
  ];
  const methods=['direct','corsproxy','allorigins'];
  for(let i=0;i<scrapers.length;i++){
    try{
      const r=await scrapers[i]();
      const h=await r.text();
      if(h.length>2000){html=h;scrapeMethod=methods[i];break;}
    }catch(e){}
  }
  ...
  if(html){
    ...
    const isRasq  = html.includes('085 25 39 03')||html.includes('085/25')||hl.includes('trevirasquain')||hl.includes('trevi rasquain')||hl.includes('info@trevirasquain');
    if(isLiege&&!isRasq) agenceIndex=1;
    else if(isRasq&&!isLiege) agenceIndex=2;
    else if(isLiege&&isRasq){
    ...
    // ── CLAUDE ─────────────────────────────────────────────────────────────────
    try{
      const SYSTEM=`Tu es expert en communication immobilière belge pour TREVI Rasquain et TREVI Liège.
...
3. Adapte intelligemment la structure au TYPE de bien (maison, appart, terrain, commerce, industriel, viager, location)
4. Chaque bien est unique — sois créatif dans l'accroche tout en restant factuel
5. RETOURS À LA LIGNE : utilise \\n pour séparer les éléments, une ligne vide (\\n\\n) entre chaque section

═══════════════════════════════════════════
FORMAT POST FACEBOOK — STRICT
═══════════════════════════════════════════

[Si exclusivité mentionnée dans le contenu]
...
🏡 𝗔 𝗩𝗘𝗡𝗗𝗥𝗘 – [description type de bien] à [𝗩𝗜𝗟𝗟𝗘 𝗘𝗡 𝗚𝗥𝗔𝗦] 📌
[ou 🏡 𝗔 𝗟𝗢𝗨𝗘𝗥 pour une location]

${vv?`🎥 VISITE VIRTUELLE DISPONIBLE : ${vv}\n`:''}
[ACCROCHE : 2-3 phrases valorisantes. Surface/terrain en gras : 𝟭𝟱𝟬 𝗺². Mots clés en gras si pertinent.]

✨ 𝗖𝗼𝗺𝗽𝗼𝘀𝗶𝘁𝗶𝗼𝗻 𝗱𝘂 𝗯𝗶𝗲𝗻 :
[Si niveaux disponibles → sous-titres ✔️ :]
✔️ 𝗦𝗼𝘂𝘀-𝘀𝗼𝗹 :
– [élément]
✔️ 𝗥𝗲𝘇-𝗱𝗲-𝗰𝗵𝗮𝘂𝘀𝘀𝗲́𝗲 :
– [élément]
...
[Commerce/Industriel → ✨ 𝗖𝗼𝗺𝗽𝗼𝘀𝗶𝘁𝗶𝗼𝗻 + ⚡️ 𝗜𝗻𝗳𝗼𝘀 𝗽𝗿𝗮𝘁𝗶𝗾𝘂𝗲𝘀 :]

🌿 𝗘𝘅𝘁𝗲́𝗿𝗶𝗲𝘂𝗿𝘀 :
– [si extérieurs disponibles]

⚡️ 𝗜𝗻𝗳𝗼𝘀 𝘁𝗲𝗰𝗵𝗻𝗶𝗾𝘂𝗲𝘀 :
– PEB : [LETTRE] (𝗫𝗫𝗫 kWh/m²/an)
– Chauffage : [type]
– Châssis : [type]
– [TOUS les autres éléments disponibles : panneaux PV, PAC, VMC, citerne, alarme, clim, boiler, adoucisseur, RC, vidéophone…]
– Électricité conforme [ou non conforme]
[⚠️ Actuellement loué — obligations locatives à respecter] [si applicable]
[✅ Libre à l'acte !] [si applicable]
...
📅 [Disponible le … — X mois de caution]
📋 Candidature à : [email si disponible]
[ou viager :]
...
${isCDC?`
═══════════════════════════════════════════
FORMAT POST INSTAGRAM — APRÈS LE POST FB
═══════════════════════════════════════════
Après le post Facebook, ajoute exactement "---INSTAGRAM---" sur une ligne seule, puis le post Instagram :

🏡 𝗔 𝗩𝗘𝗡𝗗𝗥𝗘 – [Type de bien] · [𝗩𝗶𝗹𝗹𝗲]
[ou 🏡 𝗔 𝗟𝗢𝗨𝗘𝗥]

✨ [surface] · [X ch] · PEB [lettre] · [2-3 atouts majeurs en bullet courts]

[1 phrase d'accroche percutante + emoji]

💰 [prix court] €
[ou FO à partir de X.XXX € / Loyer X.XXX €/mois]

📩 ${contactEmail} · 📞 ${contactTel}
`:''}
Séparateur d'adresse : après TOUT le contenu, ajoute "---ADRESSE---" puis l'adresse complète trouvée (rue + numéro + code postal + ville), ou "INCONNU" si introuvable.

═══════════════════════════════════════════
CARACTÈRES UNICODE GRAS — OBLIGATOIRES
═══════════════════════════════════════════
Chiffres : 𝟬𝟭𝟮𝟯𝟰𝟱𝟲𝟳𝟴𝟵
...
MISE EN PAGE :
- Tiret – (demi-cadratin U+2013), jamais le trait d'union -
- Ligne vide \\n\\n entre chaque section principale
- Jamais de bullet points •
- Chaque élément de liste sur sa propre ligne`;
      ...
      const full=aiData?.content?.[0]?.text?.trim()||'';

      // Parser la réponse : FB --- INSTAGRAM --- ADRESSE
      const parts=full.split('---ADRESSE---');
      ...
      const contentParts=parts[0].split('---INSTAGRAM---');
      postFb=contentParts[0]?.trim()||null;
      if(isCDC&&contentParts[1]) postIg=contentParts[1].trim();
  ...
  try{
    ...
    const colVals={
      dropdown_mkxvvrvk:{ids:isCDC?[2]:[1]}, // Fb+Ig pour CDC, Fb seul sinon
    ...
    const itemId=cd?.data?.create_item?.id;
    if(!itemId) throw new Error('create_item: '+JSON.stringify(cd?.errors||cd));
    ...
    const u1=[
      ...
      allPhotos[0]?`<p>📸 <img src="${allPhotos[0]}" style="max-width:100%;border-radius:8px;" /></p>`:'',
    ].filter(Boolean).join('');
    await mondayQ(`mutation{create_update(item_id:${itemId},body:${JSON.stringify(u1)}){id}}`);
    ...
    // ── UPDATE 3 : POST INSTAGRAM (Coup de cœur seulement) ──────────────────────
    if(isCDC){
      const u3=postIg
    ...
    if(allPhotos.length>0){
      const photosHtml=allPhotos.map((p,i)=>
        `<p><strong>Photo ${i+1}</strong> — <a href="${p}" target="_blank">⬇️ Télécharger</a><br/><img src="${p}" style="max-width:100%;border-radius:6px;margin-top:4px;" /></p>`
      ).join('');
